...
void MatrixMultiplyOMP(const ellpack* A, const double* B, double*  C, int chunk){
    int i, j;
    double t0, t1, t2, t3;
#pragma omp parallel for shared(A, B, C) private(i, j, t0, t1, t2, t3) schedule(static, chunk)
    for (i = 0; i < A->M - 3; i = i + 4) {
...
//    fprintf(stdout,"ID: 100, Unroll4V\n");
...
//            fprintf(stdout,"Max error: %lf\n",max_error);
            fprintf(stdout,"ELLPACK Unroll 4V \t Time: %lf \t MFLOPS %lf \t GFLOPS %lf \t Error %lf \t CHUNK %d \n",tmlt,mflops,mflops/1000,max_error,CHUNK[id_chunk]);
            }
        }
