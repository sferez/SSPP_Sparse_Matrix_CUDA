...
void MatrixMultiplyOMP(const csr* A, const double* B, double*  C, int chunk) {
    ...
    int i, j0,j1, j2, j3;
    double t0, t1, t2, t3;
#pragma omp parallel for shared(C,B,A) private(i,j0,j1,j2,j3,t0,t1,t2,t3) schedule(static,chunk)
    for (i = 0; i < A->M - 3; i += 4) {
        t0 = 0;
        t1 = 0;
        t2 = 0;
...
//            fprintf(stdout,"Max error: %lf\n",max_error);
            fprintf(stdout,"CSR Unroll 4V \t Time: %lf \t MFLOPS %lf \t GFLOPS %lf \t Error %lf \t CHUNK %d \n",tmlt,mflops,mflops/1000,max_error,CHUNK[id_chunk]);
            }
        }
