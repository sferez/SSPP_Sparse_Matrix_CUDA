...
#pragma omp parallel for shared(C,B,A) private(i,j,t0,t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15) schedule(static,chunk)
    for (i = 0; i < A->M ; i ++) {
        ...
        j = A->IRP[i];
        if (A->IRP[i + 1] - A->IRP[i] >= 16)
            for (; j < A->IRP[i + 1] - 15; j += 16) {
                t0 += B[A->JA[j]] * A->AS[j];
                ...
                t8 += B[A->JA[j + 8]] * A->AS[j + 8];
                t9 += B[A->JA[j + 9]] * A->AS[j + 9];
                t10 += B[A->JA[j + 10]] * A->AS[j + 10];
                t11 += B[A->JA[j + 11]] * A->AS[j + 11];
                t12 += B[A->JA[j + 12]] * A->AS[j + 12];
                t13 += B[A->JA[j + 13]] * A->AS[j + 13];
                t14 += B[A->JA[j + 14]] * A->AS[j + 14];
                t15 += B[A->JA[j + 15]] * A->AS[j + 15];
            }
        for (; j < A->IRP[i + 1]; j++) {
            t0 += B[A->JA[j]] * A->AS[j];
        }
...
//            fprintf(stdout,"Max error: %lf\n",max_error);
            fprintf(stdout,"CSR Unroll 16H \t Time: %lf \t MFLOPS %lf \t GFLOPS %lf \t Error %lf \t CHUNK %d \n",tmlt,mflops,mflops/1000,max_error,CHUNK[id_chunk]);
        }
    }
