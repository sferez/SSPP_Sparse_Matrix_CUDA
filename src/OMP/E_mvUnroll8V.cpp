...
#pragma omp parallel for shared(A, B, C) private(i, j, t0, t1, t2, t3, t4, t5, t6, t7) schedule(static, chunk)
    for (i = 0; i < A->M - 7; i = i + 8) {
        t0 = 0.0, t1 = 0.0, t2 = 0.0, t3 = 0.0, t4 = 0.0, t5 = 0.0, t6 = 0.0, t7 = 0.0;
        for (j = 0; j < A->max_nz; j++) {
            if (A->JA[i][j] == -1 && A->JA[i + 1][j] == -1 && A->JA[i + 2][j] == -1 && A->JA[i + 3][j] == -1 &&
                A->JA[i + 4][j] == -1 && A->JA[i + 5][j] == -1 && A->JA[i + 6][j] == -1 && A->JA[i + 7][j] == -1) {
                j = A->max_nz;
            } else {
                t0 += A->AS[i][j] * B[A->JA[i][j]];
                t1 += A->AS[i + 1][j] * B[A->JA[i + 1][j]];
                t2 += A->AS[i + 2][j] * B[A->JA[i + 2][j]];
                t3 += A->AS[i + 3][j] * B[A->JA[i + 3][j]];
                t4 += A->AS[i + 4][j] * B[A->JA[i + 4][j]];
                t5 += A->AS[i + 5][j] * B[A->JA[i + 5][j]];
                t6 += A->AS[i + 6][j] * B[A->JA[i + 6][j]];
                t7 += A->AS[i + 7][j] * B[A->JA[i + 7][j]];
            }
        }
        C[i] = t0;
        C[i + 1] = t1;
...
//            fprintf(stdout,"Max error: %lf\n",max_error);
            fprintf(stdout,"ELLPACK Unroll 8V \t Time: %lf \t MFLOPS %lf \t GFLOPS %lf \t Error %lf \t CHUNK %d \n",tmlt,mflops,mflops/1000,max_error,CHUNK[id_chunk]);
            }
        }
