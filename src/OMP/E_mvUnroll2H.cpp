...
void MatrixMultiplyOMP(const ellpack* A, const double* B, double*  C, int chunk)
{
    ...
    int i, j;
    double t0, t1;
#pragma omp parallel for shared(C,B,A) private(i,j,t0,t1) schedule(static,chunk)
    for (i = 0; i < A->M ; i ++) {
        t0 = 0, t1 = 0;
        for ( j=0 ; j < A->max_nz - 1; j += 2) {
            if (A->JA[i][j] == -1) {
                j= A->max_nz;
            } else{
                t0 += A->AS[i][j] * B[A->JA[i][j]];
...
//    fprintf(stdout,"ID: 100, Unroll2H\n");
...
//            fprintf(stdout,"Max error: %lf\n",max_error);
            fprintf(stdout,"ELLPACK Unroll 2H \t Time: %lf \t MFLOPS %lf \t GFLOPS %lf \t Error %lf \t CHUNK %d \n",tmlt,mflops,mflops/1000,max_error,CHUNK[id_chunk]);
        }
    }
