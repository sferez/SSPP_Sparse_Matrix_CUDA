...
__global__ void gpuMatrixVector(const int *IRP, const int *JA, const double* AS, const double* B, double*  C, int M) {
    ...
    if (row < M) {
        for (i = offset + tc; i < (end-XBD*3); i += XBD*4) {
            sdata[tc] += AS[i] * B[JA[i]] + AS[i+XBD] * B[JA[i+XBD]] + AS[i+XBD*2] * B[JA[i+XBD*2]] + AS[i+XBD*3] * B[JA[i+XBD*3]];
        }
        for (; i < end; i += XBD) {
            sdata[tc] += AS[i] * B[JA[i]];
        }
    }
    __syncthreads();
...
int main(int argc, char** argv) {
    ...
    std::cout << "CSR CUDA 1DB 1DG Unroll 4H\t Time " << bestTime << " ms \t GFLOPS " << gpuflops << " \t Max error " << max_error << "\t Block size " << XBD << "x1 " << "Grid size " << GRID_DIM.x << "x" << GRID_DIM.y << std::endl;
