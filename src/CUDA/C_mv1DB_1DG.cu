...
int main(int argc, char** argv) {
    ...
    std::cout << "CSR CUDA 1DB 1DG \t\t Time " << bestTime << " ms \t GFLOPS " << gpuflops << " \t Max error " << max_error << "\t Block size " << XBD << "x1 " << "Grid size " << GRID_DIM.x << "x" << GRID_DIM.y << std::endl;
