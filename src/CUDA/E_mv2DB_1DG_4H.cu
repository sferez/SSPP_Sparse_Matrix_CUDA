...
__global__ void gpuMatrixVector(const int *JA, const double* AS, const double* B, double*  C, int M, int max_nz) {
    ...
    if (row < M) {
        ...
        int ic   = tc;
        double t  = 0.0;
        for ( ; ic < (max_nz-XBD*3); ic += XBD*4) {
            if (JA[idxm] == -1) break;
            t += AS[idxm] * B[JA[idxm]] + AS[idxm+XBD] * B[JA[idxm+XBD]] + AS[idxm+XBD*2] * B[JA[idxm+XBD*2]] + AS[idxm+XBD*3] * B[JA[idxm+XBD*3]];
            idxm += XBD*4;
        }
...
int main(int argc, char** argv) {
    ...
    std::cout << "ELLPACK CUDA 2DB 1DG Unroll 4H\t Time " << bestTime << " ms \t GFLOPS " << gpuflops << " \t Max error " << max_error << "\t Block size " << XBD << "x" << YBD << "Grid size " << GRID_DIM.x << "x" << GRID_DIM.y << std::endl;
