...
__global__ void gpuMatrixVector(const int *JA, const double* AS, const double* B, double* C, int M, int max_nz) {
    ...
    if (row < M) {
        ...
        int idxm = row ;
        double t = 0.0;
            for (int i=0 ; i < max_nz-1; i=i+2) {
            if (JA[idxm] == -1) break;
            t += AS[idxm] * B[JA[idxm]] + AS[idxm + M] * B[JA[idxm + M]];
            idxm += M*2;
        }
        for (int i=0 ; i < max_nz%2; i++) {
            if (JA[idxm] == -1) break;
            t += AS[idxm] * B[JA[idxm]];
            idxm += M;
        }
...
int main(int argc, char** argv) {
    ...
    std::cout << "ELLPACK CUDA 1 Thread/row Transposed Unroll 2H\t\t Time " << bestTime << " ms \t GFLOPS " << gpuflops << " \t Max error " << max_error << "\t Block size " << XBD << "x1 " << "Grid size " << GRID_DIM.x << "x" << GRID_DIM.y << std::endl;
