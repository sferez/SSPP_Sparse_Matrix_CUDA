...
__global__ void gpuMatrixVector(const int *IRP, const int *JA, const double* AS, const double* B, double*  C, int M) {
    ...
    if (row < M) {
        for (i = offset + tc; i < (end-XBD); i += XBD*2) {
            sdata[tc] += AS[i] * B[JA[i]] + AS[i+XBD] * B[JA[i+XBD]];
        }
        for (; i < end; i += XBD) {
...
int main(int argc, char** argv) {
    ...
    float gpuflops = 2.e-6*A->nz/ bestTime;
    ...
    std::cout << "CSR CUDA 1DB 1DG Unroll 2H\t Time " << bestTime << " ms \t GFLOPS " << gpuflops << " \t Max error " << max_error << "\t Block size " << XBD << "x1 " << "Grid size " << GRID_DIM.x << "x" << GRID_DIM.y << std::endl;
