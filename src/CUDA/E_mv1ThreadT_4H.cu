...
__global__ void gpuMatrixVector(const int *JA, const double* AS, const double* B, double* C, int M, int max_nz) {
    ...
    if (row < M) {
        // Starting address of indexing within transposed matrix A
        int idxm = row ;
        double t = 0.0;
            for (int i=0 ; i < max_nz-3; i=i+4) {
            if (JA[idxm] == -1) break;
            t += AS[idxm] * B[JA[idxm]] + AS[idxm + M] * B[JA[idxm + M]] + AS[idxm + M*2] * B[JA[idxm + M*2]] + AS[idxm + M*3] * B[JA[idxm + M*3]];
            idxm += M*4;
        }
        for (int i=0 ; i < max_nz%4; i++) {
            if (JA[idxm] == -1) break;
            t += AS[idxm] * B[JA[idxm]];
            idxm += M;
        }
...
int main(int argc, char** argv) {
    ...
    dim3 GRID_DIM = (nrowsc + BLOCK_DIM.x - 1) / BLOCK_DIM.x;
    ...
    std::cout << "ELLPACK CUDA 1 Thread/row Transposed Unroll 4H\t\t Time " << bestTime << " ms \t GFLOPS " << gpuflops << " \t Max error " << max_error << "\t Block size " << XBD << "x1 " << "Grid size " << GRID_DIM.x << "x" << GRID_DIM.y << std::endl;
